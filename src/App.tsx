...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      showAlert("error", "Koneksi ke server gagal. Pastikan backend server berjalan.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center space-x-4">
            <div className="text-right mr-4 hidden md:block">
              ...
              <div className="text-sm font-mono font-medium text-slate-700">
                {config.botUsername ? `@${config.botUsername}` : "@not_configured"}
              </div>
            </div>
            <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center text-slate-600 font-semibold uppercase">
              {config.botUsername ? config.botUsername.substring(0, 2) : "TG"}
            </div>
          </div>
        </header>
